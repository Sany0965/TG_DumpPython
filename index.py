...
async def generate_index(user, dialogs, output_dir="dialogs"):
    # Сортировка: сначала Пользователь/Избранное, потом Каналы и Чаты, затем Боты
    order = {
        'Пользователь': 0,
        'Избранное': 0,
        'Канал': 1,
    ...
    with open(index_path, 'w', encoding='utf-8') as f:
        ...
        f.write('.user-details { line-height: 1.5; flex: 1; min-width: 200px; }')
        ...
        f.write('.dialog-icon { width: 36px; height: 36px; display: flex; align-items: center; justify-content: center; background: #00e571; color: #1e1e1e; font-size: 20px; font-weight: bold; border-radius: 50%; margin-right: 12px; }')
        ...
        f.write(f'<p><strong>ID:</strong> {user.get("id", "Неизвестно")}</p>')
        f.write(f'<p><strong>Username:</strong> {user.get("username", "Неизвестно")}</p>')
        f.write(f'<p><strong>Номер:</strong> {user.get("phone", "Неизвестно")}</p>')
        f.write(f'<p><strong>Био:</strong> {user.get("bio", "")}</p>')
        premium_text = "Да" if user.get("is_premium") else "Нет"
        f.write(f'<p><strong>Премиум:</strong> {premium_text}</p>')
        if user.get("devices"):
            f.write('<div class="devices">')
            f.write('<h3>Подключённые устройства</h3>')
            f.write('<table>')
            f.write('<tr><th>Модель</th><th>Платформа</th><th>Версия</th><th>IP</th></tr>')
            for dev in user["devices"]:
                f.write(f'<tr><td>{dev.get("device_model", "")}</td><td>{dev.get("platform", "")}</td><td>{dev.get("app_version", "")}</td><td>{dev.get("ip", "")}</td></tr>')
            f.write('</table>')
            f.write('</div>')
        f.write('</div>')
        f.write('</div>')
        ...
        for dialog in dialogs_sorted:
            ...
            f.write(f'''
            <div class="dialog">
                <div class="dialog-icon">{initial_letter}</div>
